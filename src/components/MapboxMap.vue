  ...
  <div class="map-shell">
    ...
    <div class="tool-panel-wrap">
      ...
      <div class="tool-panel" v-if="panelOpen">
        ...
        <button
          class="tool-item"
          :class="{ active: activeMode === 'draw' && drawSubTool === 'polygon' }"
        ...
        <button
          class="tool-item"
          :class="{ active: activeMode === 'measure' && drawSubTool === 'line' }"
        ...
        <button
          class="tool-item"
          :class="{ active: activeMode === 'measure' && drawSubTool === 'polygon' }"
...
const presetColors  = ["#e74c3c","#e67e22","#f1c40f","#2ecc71","#3498db","#9b59b6","#1a1a1a"];
...
const DRAWN_LAYERS = ["drawn-polygon-fill","drawn-polygon-stroke","drawn-line","drawn-point"];
...
function initDrawnSource() {
  ...
  // پلیگان fill
  map.addLayer({
    id: "drawn-polygon-fill", type: "fill", source: DRAWN_SRC,
  ...
  // پلیگان stroke
  map.addLayer({
    id: "drawn-polygon-stroke", type: "line", source: DRAWN_SRC,
    filter: ["==", ["geometry-type"], "Polygon"],
  ...
  // خط
  map.addLayer({
    id: "drawn-line", type: "line", source: DRAWN_SRC,
  ...
  // نقطه
  map.addLayer({
    id: "drawn-point", type: "circle", source: DRAWN_SRC,
...
function makeDrawStyles(color) {
  return [
    ...
    },
    {
      ...
      paint: { "circle-radius": 4, "circle-color": color, "circle-opacity": 0.6 },
    },
...
function getFeatureCenter(feature) {
  try { return turf.centerOfMass(feature).geometry.coordinates; }
  catch { return null; }
}
...
function updateLabels() {
  ...
  drawnFeatures.forEach((f) => {
    ...
    else if (f.geometry.type === "Polygon" && f.geometry.coordinates[0]?.length >= 4)
...
function updateLiveLabel() {
  ...
  if (live.geometry.type === "LineString")
    ...
    try { const a = turf.area(live); if (a > 0) text = formatArea(a); } catch {}
  }
  if (!text) return;
...
function createDrawInstance(mode, subTool) {
  const controls =
    mode === "draw"
      ...
      : { line_string: subTool === "line", polygon: subTool === "polygon", trash: true };
...
function removeDraw() {
  ...
  try { if (draw && map.hasControl(draw)) map.removeControl(draw); } catch {}
  draw = null;
}
...
// ─── رندر عوارض ───────────────────────────────────────────
const COLORS_WELL = ["#c97a4a","#4a9b8e","#d4a546","#8b7bb8","#c0563f","#5b9bd5","#7ec88a"];
...
function clearWellLayers() {
  ["wells-fill", "wells-line", "wells-polyline", "wells-point"].forEach((id) => {
...
function popupHTML(w, props_) {
  const entries = Object.entries(props_)
    ...
    .slice(0, 8)
    .map(([k, v]) => `<tr><td style="color:#9aab9f;padding:2px 8px 2px 0">${k}</td><td>${v ?? "—"}</td></tr>`)
    .join("");
...
function renderMarkers() {
  ...
  if (hasGeometry) {
    ...
    geojson.features.forEach((f) => {
      ...
      f.properties._highlighted = highlightSet.has(String(f.properties.id)) ? 1 : 0;
    });
    map.addSource("wells-src", { type: "geojson", data: geojson });
    map.addLayer({
      id: "wells-fill", type: "fill", source: "wells-src",
    ...
    map.addLayer({
      id: "wells-line", type: "line", source: "wells-src",
      filter: ["in", ["geometry-type"], ["literal", ["Polygon", "MultiPolygon"]]],
      paint: {
    ...
    map.addLayer({
      id: "wells-polyline", type: "line", source: "wells-src",
    ...
    map.addLayer({
      id: "wells-point", type: "circle", source: "wells-src",
      filter: ["in", ["geometry-type"], ["literal", ["Point", "MultiPoint"]]],
      paint: {
        "circle-radius":       ["case", ["==", ["get", "_highlighted"], 1], 9, 6],
        "circle-color":        ["get", "_color"],
        "circle-stroke-width": 2,
        "circle-stroke-color": ["case", ["==", ["get", "_highlighted"], 1], "#1a1a1a", "#ffffff"],
      },
    });
    ["wells-fill", "wells-line", "wells-polyline", "wells-point"].forEach((layerId) => {
    ...
    try {
      ...
      if (!bounds.isEmpty()) map.fitBounds(bounds, { padding: 48, maxZoom: 14, duration: 800 });
    } catch {}
  } else {
    props.wells.forEach((w) => {
      ...
      const entries = Object.entries(w)
        ...
        .map(([k, v]) => `<tr><td style="color:#9aab9f;padding:2px 8px 2px 0">${k}</td><td>${v ?? "—"}</td></tr>`)
        .join("");
      const popup = new mapboxgl.Popup({ offset: 14, maxWidth: "280px" }).setHTML(`
        <div style="font-family:'Vazirmatn',sans-serif;direction:rtl;min-width:200px">
          <div style="font-weight:700;margin-bottom:6px;font-size:13px">عارضه #${w.id}</div>
          <table style="font-size:12px;line-height:1.8;width:100%">${entries}</table>
        </div>`);
      el.addEventListener("click", (e) => {
        if (activeMode.value) { e.stopPropagation(); return; }
        emit("select-well", w);
      });
      markers.push(
        new mapboxgl.Marker(el).setLngLat([w.lng, w.lat]).setPopup(popup).addTo(map)
      );
    });
...
function renderRadiusAndLines() {
  ...
  ["radius-fill","radius-line","center-point","neighbor-lines"].forEach((id) => {
    if (map.getLayer(id)) map.removeLayer(id);
  });
  ...
  if (props.radiusCenter && props.radiusKm > 0) {
    ...
    map.addSource("radius-src", { type: "geojson", data: { type: "Feature", geometry: circle } });
    map.addLayer({ id: "radius-fill", type: "fill", source: "radius-src",
      paint: { "fill-color": "#4a9b8e", "fill-opacity": 0.08 } });
    map.addLayer({ id: "radius-line", type: "line", source: "radius-src",
      paint: { "line-color": "#4a9b8e", "line-width": 1.5, "line-dasharray": [2, 2] } });
    map.addSource("center-src", { type: "geojson",
      data: { type: "Feature", geometry: { type: "Point", coordinates: [props.radiusCenter.lng, props.radiusCenter.lat] } } });
    map.addLayer({ id: "center-point", type: "circle", source: "center-src",
      paint: { "circle-radius": 7, "circle-color": "#e9efe9", "circle-stroke-width": 3, "circle-stroke-color": "#4a9b8e" } });
  }
  if (props.neighborPairs.length) {
    const lines = {
      ...
      features: props.neighborPairs
        ...
        .map((pair) => ({
          type: "Feature",
          geometry: { type: "LineString", coordinates: [[pair.a.lng, pair.a.lat], [pair.b.lng, pair.b.lat]] },
        })),
    };
    map.addSource("neighbor-src", { type: "geojson", data: lines });
    map.addLayer({ id: "neighbor-lines", type: "line", source: "neighbor-src",
      paint: { "line-color": "#d4a546", "line-width": 1.5, "line-dasharray": [1, 2], "line-opacity": 0.7 } });
  }
}
...
watch(() => [props.radiusCenter, props.radiusKm], renderRadiusAndLines, { deep: true });
...
defineExpose({
  ...
  zoomToFeature(id) {
    ...
    if (well._geometry) {
      ...
      if (g.type === "Point") { map.flyTo({ center: g.coordinates, zoom: 13, duration: 800 }); return; }
      const bounds = new mapboxgl.LngLatBounds();
      if (g.type === "LineString") g.coordinates.forEach((c) => bounds.extend(c));
      else if (g.type === "MultiLineString") g.coordinates.forEach((l) => l.forEach((c) => bounds.extend(c)));
      else if (g.type === "Polygon") g.coordinates[0].forEach((c) => bounds.extend(c));
      else if (g.type === "MultiPolygon") g.coordinates.forEach((p) => p[0].forEach((c) => bounds.extend(c)));
      if (!bounds.isEmpty()) map.fitBounds(bounds, { padding: 60, maxZoom: 15, duration: 800 });
  ...
  fitToWells(wells) {
    ...
    setTimeout(() => {
      const bounds = new mapboxgl.LngLatBounds();
      wells.forEach((w) => {
        if (!w._geometry) { if (w.lat && w.lng) bounds.extend([w.lng, w.lat]); return; }
        const g = w._geometry;
        if (g.type === "Point")            bounds.extend(g.coordinates);
        else if (g.type === "MultiPoint")  g.coordinates.forEach((c) => bounds.extend(c));
        else if (g.type === "LineString")  g.coordinates.forEach((c) => bounds.extend(c));
      ...
      if (!bounds.isEmpty()) map.fitBounds(bounds, { padding: 48, maxZoom: 14, duration: 1000 });
    }, 300);
  },
  enablePointPicker(callback) {
    ...
    wl.forEach((id) => { if (map.getLayer(id)) map.setLayoutProperty(id, "visibility", "none"); });
    const handler = (e) => {
      wl.forEach((id) => { if (map.getLayer(id)) map.setLayoutProperty(id, "visibility", "visible"); });
      map.getCanvas().style.cursor = "";
      map.off("click", handler);
      if (map._pickerMarker) map._pickerMarker.remove();
      const el = document.createElement("div");
      el.style.cssText = "width:16px;height:16px;border-radius:50%;background:#4a9b8e;border:3px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,0.4);pointer-events:none;";
      map._pickerMarker = new mapboxgl.Marker(el).setLngLat(e.lngLat).addTo(map);
  ...
  disablePointPicker() {
    if (!map) return;
    map.getCanvas().style.cursor = "";
    if (map._pickerHandler) { map.off("click", map._pickerHandler); map._pickerHandler = null; }
    if (map._pickerMarker)  { map._pickerMarker.remove(); map._pickerMarker = null; }
    ["wells-fill","wells-line","wells-polyline","wells-point"].forEach((id) => {
...
.tool-toggle-btn:hover  { border-color: #4a9b8e; background: #f0f7f5; }
.tool-toggle-btn.active { background: #4a9b8e; color: #fff; border-color: #3a8070; }
.tool-toggle-btn.active svg { stroke: #fff; }
...
.tool-divider { height: 1px; background: #eee; margin: 4px 4px; }
...
.tool-item:hover         { background: #f0f7f5; color: #2d7a6e; }
.tool-item.active        { background: #e8f5f2; color: #2d7a6e; font-weight: 600; }
.tool-item.danger        { color: #c0563f; }
.tool-item.danger:hover  { background: #fdf0ee; }
